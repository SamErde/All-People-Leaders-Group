...
function Update-PeopleLeadersGroup {
    ...
    if ($AddToGroup) {
        ...
        try {
            ...
            Write-LogFile "$($PeopleLeaders.Count) people leaders added to `"$PeopleLeadersGroup`"."
        } catch {
            Write-LogFile "A problem occured while adding users to `"$PeopleLeadersGroup`"."
        }
    } else {
    ...
    if ($RemoveFromGroup) {
        ...
        Write-LogFile "$($RemoveFromGroup.Count) users removed from `"$PeopleLeadersGroup`": "
        ...
        Write-LogFile "0 users removed from `"$PeopleLeadersGroup`"."
    }
    Write-LogFile -Log '-----------------'
